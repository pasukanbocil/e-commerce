...
@section('content')
    ...
    <div class="container mx-auto px-4 py-6">
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            @foreach ($categories as $category)
                <div class="flex flex-col items-center bg-white rounded-lg shadow-md p-4 hover:grow hover:shadow-sm">
                    <a href="#" class="flex flex-col items-center">

                        <img class="w-16 h-16 object-cover rounded-full mb-3 border border-gray-200"
                            src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->category_name }}">

                        <p class="text-sm font-semibold text-gray-700 text-center">{{ $category->category_name }}</p>
                    </a>
                </div>
            @endforeach
        </div>
    ...
    <div class="container mx-auto flex items-center flex-wrap pt-4 pb-12">
        ...
        @foreach ($products as $product)
            <a href="/product/{{ $product->slug }}/detail">
                <div class="w-full md:w-1/3 xl:w-1/4 p-6 flex flex-col">
                    <img class="hover:grow hover:shadow-lg w-ful h-400 w-400"
                        src="{{ asset('storage/' . json_decode($product->image)[0]) }}">
                    ...
                    <p class="pt-1 text-gray-900">Rp. {{ number_format($product->price, 0, ',', '.') }}</p>
                    <div class="flex items-center justify-center mt-4">
                        <a href="/product/{{ $product->slug }}/detail"
                            class="items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg shadow hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-blue-200 hover:grow hover:shadow-lg">
                            View Details
                        </a>
                    </div>
                </div>
